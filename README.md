...
4. While in ResumeProjectMain folder, run 'php app/console doctrine:migrations:migrate' to set a basic database with group permissions and some dummy data for testing (*not really up to date go to 5 of "Things that I know are missing:"*)
...
5. Doctrine migrations don't cover all the data in the DB (there's a DB dump in the _docs folder)